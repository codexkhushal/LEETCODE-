...
public:
    ...
    void generateColumnStates(string currentColumn, int rowsRemaining,
                              char prevColor) {
        if (rowsRemaining == 0) {
        ...
        for (char color : {'R', 'G', 'B'}) {
            ...
            generateColumnStates(currentColumn + color, rowsRemaining - 1,
                                 color);
        }
    }
    ...
    int solve(int remainingCols, int prevColumnIdx, int m) {
        ...
        for (int nextColumnIdx = 0; nextColumnIdx < columnStates.size();
             nextColumnIdx++) {
            ...
            if (valid) {
                totalWays =
                    (totalWays + solve(remainingCols - 1, nextColumnIdx, m)) %
                    MOD;
            }
        }
